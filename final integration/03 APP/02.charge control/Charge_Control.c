...
/********************************************************/
 ...
 void Charge_Control_init(void)
 {
	...
	for (u8 i= 0 ; i<NUM_OF_BRANCHES;i++)
	{
	  ...
	  STP_init();
	    ...
	    MGPIO_VidSetPinDirection(GPIOA,PIN12,OUTPUT_SPEED_2MHZ_PP);
	    MGPIO_VidSetPinDirection(GPIOA,PIN11,OUTPUT_SPEED_2MHZ_PP);

	    MGPIO_VidSetPinValue(GPIOA,PIN1, LOW);
	    MGPIO_VidSetPinValue(GPIOA,PIN12, LOW);
	    MGPIO_VidSetPinValue(GPIOA,PIN11, LOW);
 ...
 void Charge_Control_calcStateOfCharge(void)
 {
	...
	for(u8 i =0 ;i<NUM_OF_CELLS;i++)
	{
		...
		/*******************************************************/
	/*	if(isDisCharge[i/2])
			SOC = (float)(132.3*(cells[i].Voltage )-450.8);
		else*/
		if (cells[i].Voltage > 4.16)
		{
			cells[i].Voltage = 4.16;
		}
